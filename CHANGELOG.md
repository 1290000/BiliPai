...
[优化] 建立全局 CPU 节能调度与后台深度内存修剪机制（BackgroundMemoryTrimPolicy），在进入后台与系统内存紧张时主动释放连接池、图片缓存与弹幕视图资源
[优化] 完善小窗播放（PiP / MiniPlayer）与后台活动保护机制，避免过度回收导致的重连开销

## 界面与液态玻璃

[优化] 统一全局浮层与半屏抽屉渲染架构，居中弹窗接入底栏液态玻璃通透质感与边沿折射
[修复] 完善单选项设置展示方式支持：选择“跟随选项弹出”使用 Miuix 原生 WindowSpinnerPreference 就地弹出；选择“居中弹窗”走液态玻璃通透渲染
