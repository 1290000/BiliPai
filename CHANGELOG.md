...
[更改] 版本号 0.2.3-beta.36，versionCode 360
[添加] 直播醒目留言浮层可独立跟随弹幕开关控制 @cunminF
[添加] 设置支持保存自定义配置并一键恢复；新用户首次启动按内置配置初始化
[修复] 首页、搜索、空间、列表等页面切换时的顶部主题色闪变与液态玻璃状态断层
[更改] 首页横幅与视频转场实时模糊默认关闭
[更改] 视频小横条进入详情页使用来源快照参与普通返回交接；预测性返回暂不接入该来源
[修复] 平板与宽屏下空间、视频卡片和导航布局的尺寸与切换问题
v0.2.3-beta.36(cc5e509)
