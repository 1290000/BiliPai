...
[修复] 全屏与非全屏共享弹幕显示开关、字号和显示区域，兼容旧配置；字号按屏幕密度换算，短视口轨道不再超出区域限制
[优化] 收紧投票弹幕卡片并限制在播放器边界内；评分弹幕改为五颗星，按 2/4/6/8/10 分提交，缺失档位不可选
[重构] 普通、高级及互动弹幕共用播放器视口与缩放策略，非全屏叠加内容随视口缩小；同步缩放描边、轨道与卡片尺寸，保留暂停帧并重新测量
[优化] 小窗互动弹幕以紧凑预览展示，点击展开正常触摸尺寸的操作面板；切换视口和重新打开面板保留已关闭及已选择状态
