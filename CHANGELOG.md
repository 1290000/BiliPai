...
[优化] 统一播放器竖屏折叠与共享元素返回动效规范，消除多余物理回弹并锁定 16:9 标准比例高度
[添加] UP 主个人空间点赞列表支持“查看全部”与完整分页加载，深度对齐桌面端数据接口
[优化] 空间及二级分段控件回归原生主题组件，去除 MD3 播放手势音量/亮度指示器外层多余背景层
[优化] 主页视频卡片接入浅色模式暗底高对比度文字自适应，静态壁纸大半径模糊支持显存自适应降采样
[优化] 依照规范重构优化设置项分组排版与弹窗宽度适配，完善切后台壁纸调色板状态保留
