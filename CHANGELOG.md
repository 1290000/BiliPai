## v0.2.3-alpha.4 (2026-09-24)

更新日志(ChangeLog)

[更改] 版本号 0.2.3-alpha.4，versionCode 394

[优化] 分类分页滑动时，横向分类栏与指示器连续同步跟随

[优化] 听视频播放页的直线进度条不再启动无用的波形循环动画

[修复] 从播放小横条进入详情页再返回时，提前衔接卡片内容，减少黑色过渡空档

[添加] 无音频播放时，底栏与搜索条也支持下滑合体、上滑解体

[修复] 液态玻璃分享面板的材质显示和应用图标

[更改] 底部弹层拖拽把手支持显式关闭，并按需启用实验性把手 API
...
[更改] 版本号 0.2.3-alpha.3，versionCode 392

[添加] 搜索分类顺序支持自定义，分类栏切换与指示器跟随更平滑 @1290000

[添加] 视频分享支持链接与卡片两种形式，完善卡片二维码和标题信息 @1290000

[更改] 取消全局触摸刷新率干预，新增显示模式偏好，由系统与播放器管理刷新率

[更改] 首页顶栏回顶采用分段平滑滚动，覆盖推荐、直播、番剧、分区和订阅

[修复] 快速进入视频后立即返回时，加载中的卡片信息提前参与落位

[修复] 非活动个人主页的加载骨架停止呼吸动画，降低后台绘制负担

[修复] 私信回复被输入栏遮挡、横屏视频侧栏设置未生效的问题
