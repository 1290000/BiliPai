...
[修复] 修复竖屏全屏模式下点击被弹幕层拦截导致控制面板无响应、以及主线程正则计算超时引发的闪退（ANR）
[优化] 竖屏流播放时隔离并卸载内联播放器组件，防止双宿主竞争与多余后台轮询，降低运行功耗
[优化] 精细对齐视频卡片预测性返回手势与共享元素动画：平缓 3D 姿态与漂移，手势滑动过程中内容等比变小平滑收拢回原卡片，消除打断返回时的页面跳动与回弹

## 弹幕与互动

[重构] 普通、高级及互动弹幕共用播放器视口与等比缩放管线，非全屏及小窗下随视口缩小，同步缩放描边、轨道与卡片尺寸
[优化] 调整弹幕字号与外观样式时保留暂停帧画面并就地重测，无需重新载入弹幕流；全屏与非全屏共享显示开关及区域设置
[优化] 收紧投票卡片并约束在播放器视口内；评分弹幕规范为五星（2/4/6/8/10 分）直选，视口切换后保留操作与交互状态
...
[修复] 修复评论详情页主动触发反诈检测后未弹出检测结果提示的问题
