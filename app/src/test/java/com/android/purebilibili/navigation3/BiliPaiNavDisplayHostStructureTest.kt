...
class BiliPaiNavDisplayHostStructureTest {
    ...
    fun miuixHeroHasOneOwnerForGeometryEffectsAndLifetime() {
        ...
        assertTrue(source.contains("remember(transitionSourceKey) { MiuixVideoCardTransitionProgress() }"))
        assertTrue(source.contains("audioNowPlayingTransitionBvid"))
        assertTrue(source.contains("currentAudioNowPlayingVideo?.bvid ?: audioNowPlayingTransitionBvid"))
        assertTrue(source.contains("if (!isAudioNowPlayingVideoEntry)"))
